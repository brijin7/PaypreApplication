...
public partial class Master_Newforms_TrainerDetailsApproval : System.Web.UI.Page
{
	...
	public void BindTrainerDetails()
	{
...

		{
			using (var client = new HttpClient())
			{
				...
				string sUrl = Session["BaseUrl"].ToString().Trim() + "trainerDetails/GetTrainerDetailsApproval?branchId=" + Session["branchId"].ToString() + "&gymownerId=" + Session["gymownerId"].ToString();
				HttpResponseMessage response = client.GetAsync(sUrl).Result;
				...
				if (response.IsSuccessStatusCode)
				{
                    ...
                    if (StatusCode == 1)
					{
						...
						if (dt.Rows.Count > 0)
						{
                            ...
                            int myInt = dt.Rows.Count;
                            gridcount.Value =  myInt.ToString();
	...
	protected void LnkEdit_Click(object sender, ImageClickEventArgs e)
	{
		try
		{
			...
			ddlSpecialistType.Enabled=false;
    ...
    public class TrainerDetailsApprove
    {
        public string queryType { get; set; }
        public string uniqueId { get; set; }
        public string updatedBy { get; set; }
    ...
    protected void lnkActiveOrInactive_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string sActiveStatus = lblActiveStatus.Text.Trim() == "Approved" ? "Y" : "N";
            string QueryType = string.Empty;
            if (sActiveStatus.Trim() == "N")
            {
                QueryType = "Approve";
            }
            else
            ...
            using (var client = new HttpClient())
            {
                ...
                client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                client.DefaultRequestHeaders.Add("Authorization", "Bearer " + Session["APIToken"].ToString());
                var activeOrInActive = new TrainerDetailsApprove()
                {
                    queryType = QueryType.Trim(),
                    uniqueId = lbluniqueId.Text.Trim(),
                    updatedBy = Session["userId"].ToString()
                };
                HttpResponseMessage response = client.PostAsJsonAsync("trainerDetails/approveOrInApprove", activeOrInActive).Result;
                if (response.IsSuccessStatusCode)
                {
                    var Fitness = response.Content.ReadAsStringAsync().Result;
                    int StatusCode = Convert.ToInt32(JObject.Parse(Fitness)["StatusCode"].ToString());
    ...
    protected void gvTrainerDetails_RowCreated(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            int index = e.Row.RowIndex;
            Image myImage = (Image)e.Row.FindControl("lblcertificates");
            myImage.ID = "lblcertificates_" + index;
        }
    }

    protected void gvTrainerDetails_RowDataBound(object sender, GridViewRowEventArgs e)
    {
